...
#using inheritence to create random computer player and human player
...
class HumanPlayer(Player):
    ...
    def get_move(self, game):
        ...
        while not valid_square:
            ...
            """We're going to check that this is a correct value by trying
...
class GeniusComputerPlayer(Player):
    ...
    def get_move(self, game):
        if len(game.available_moves()) == 9:
            ...
            #get the square based off the mimimax algorithm
            square = self.minimax(game, self.letter)['position']
        return square

    def minimax(self, state, player): #'state' can be any name, but this is literally a screen shot of the current state of the game
        ...
        if state.current_winner == other_player:
            #we should return position AND score bc we need to keep track of the score
            # for minimax to work
